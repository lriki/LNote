...
enum MMEVariableRequest
{
    ...
    MME_VARREQ_CONTROLOBJECT_Hide,          ///< 指定したオブジェクトが表示されているか否か (bool)
    ...
    MME_VARREQ_CONTROLOBJECT_Tr,            ///< アクセサリの透明度(アクセサリパネルのSi)。型はfloat。
...
{
    Graphics::IShaderVariable*  Variable;           ///< シェーダ変数
    MMEVariableRequest          Request;            ///< 変数が要求しているものの種類
    SceneNode*                  ControlObject;      ///< CONTROLOBJECT の参照対象 (新しいオブジェクトが Scene に追加されたときに再取得する)
    LRefTString                 ObjectName;         ///< CONTROLOBJECT の name アノテーションの値
    LRefTString                 ItemName;           ///< CONTROLOBJECT の item アノテーションの値
    Graphics::ITexture*         Texture;            ///< OFFSCREENRENDERTARGET 等で作成されたテクスチャ (セマンティクスがなくても ResourceName アノテーションがあれば作る)
    lnU32                       LightNum;           ///< ライト要素を格納する変数が配列の場合、その要素数 (必要なライト数。配列でなければ 0)
...
{
    const LMatrix*   Mat_LightView;         ///< デフォルトライトのビュー行列
    const LMatrix*   Mat_LightProj;         ///< デフォルトライトのプロジェクション行列
    const LMatrix*   Mat_LightViewProj;     ///< デフォルトライトのビュー×プロジェクション行列
    //const LMatrix*   Mat_LightOrtho;        ///< (未実装)
    LVector4	    LightPosition;			///< ライトの位置
    LVector4	    LightDirection;			///< ライトの向き
    const LVector4*	LightDiffuse;		    ///< ライトのディフューズ色（拡散光）
	const LVector4*	LightAmbient;			///< ライトのアンビエント色（環境光）
	const LVector4*	LightSpecular;          ///< ライトのスペキュラ色（鏡面光）

    lnfloat         Time;                   ///< 経過時間 (秒) ←ゲーム開始からずっと加算とかだとヤバいので何か対応しておく
    lnfloat         ElapsedTime;            ///< 前回のフレームからの経過時間 (秒)
    LVector4        ViewPixelSize;          ///< シーンの仮想スクリーンサイズ
    LVector4        MousePosition;          ///< マウス座標 (x, y)
    LVector4        LeftMouseDown;          ///< マウスの左ボタン情報
    LVector4        MiddleMouseDown;        ///< マウスの中ボタン情報
    LVector4        RightMouseDown;         ///< マウスの右ボタン情報
